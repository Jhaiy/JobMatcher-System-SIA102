...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const form = document.querySelector('form[action="employer-joblisting-page.php"]');

    doneButton.addEventListener("click", (event) => {
        event.preventDefault();
        const selectedCategory = document.querySelector('input[name="category-types[]"]:checked');

        if (selectedCategory) {
            selectedCategoryList.innerHTML = "";
            const listItem = document.createElement("li");
            listItem.textContent = selectedCategory.nextElementSibling.textContent;
            selectedCategoryList.appendChild(listItem);
            const existingCategoryInput = document.querySelector('input[name="category-item"]');
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    doneButton.addEventListener("click", (event) => {
        event.preventDefault();
        const selectedRole = document.querySelector('input[name="role-types[]"]:checked');
        ...
        if (selectedRole) {
            ...
            const existingRoleInput = document.querySelector('input[name="role-item"]');
